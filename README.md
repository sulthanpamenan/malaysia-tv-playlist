...
* **Automated 24/7**: Updated every **3 hours** via GitHub Actions to ensure dynamic stream tokens never expire.
* **Integrated EPG & Logos**: Pre-configured with Astro XMLTV program guide references (`url-tvg`) and high-resolution channel logos extracted directly from the web grid.
...
* **Self-Healing Grid Scraper**: Uses Playwright to dynamically scan the source site grid. If new channels are added to the source website, they will automatically be detected and included.
...
4. The script maps channel metadata, generates standardized EPG IDs, and rebuilds the `playlist.m3u` file.
